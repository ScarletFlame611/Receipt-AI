...
пересчитывается синхронно с изображением через bbox-механизм albumentations.
"""
...
def build_augmentation_pipeline():
    """Собирает пайплайн аугментаций с синхронным пересчётом рамок.
    bbox_params задаёт формат YOLO и отбрасывает рамки, от которых после
    искажения осталось меньше 30% площади (чтобы не учить модель на
    обрезках чека).
    """
    return A.Compose(
        [
